...
describe('<FieldTemplate />', () => {
	...
	it('should render the correct className', () => {
		expect(wrapper.find(FieldWrapper).prop('className')).toBe(props.classNames + ' field-wrapper');
	});
	...
	it('should render the correct label content', () => {
		expect(wrapper.find(FieldWrapper).find('label').get(0).props.children[0]).toEqual(props.label);
		expect(wrapper.find(FieldWrapper).find('label').get(0).props.children[1]).toEqual('*');
	});
	...
	it('should render the correct description content', () => {
		expect(wrapper.find(FieldWrapper).find(Description).get(0).props.children).toBe(props.rawDescription);
	});
	...
	it('should not render \'*\' if required is false', () => {
		...
		expect(wrapper.find(FieldWrapper).find('label').get(0).props.children[1]).toBeNull();
	});
