...
describe('<CustomCheckbox />', () => {
	...
	it('<input /> should have type checkbox', () => {
		expect(wrapper.find(CheckboxLabel).find('input').prop('type')).toEqual('checkbox');
	});

	it('should have * inside <CheckboxLabel /> when field is required', () => {
		expect(wrapper.find(CheckboxLabel).find('span').get(0).props.children[2]).toEqual('*');
	});

	it('should not have * inside <CheckboxLabel /> when field is not required', () => {
		wrapper.setProps({required: false});
		expect(wrapper.find(CheckboxLabel).find('span').get(0).props.children[2]).toBeNull();
	});

	it('calls onChange prop when the input is changed', () => {
		wrapper.find(CheckboxLabel).find('input').simulate('change', {currentTarget: {checked: false}});
