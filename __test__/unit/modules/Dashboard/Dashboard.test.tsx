...
describe('<Dashboard />', () => {
	describe('#render', () => {
		...
		it('should render the correct title', () => {
			expect(wrapper.find(Nav).find(StyledNavItem).at(0).find(TitleFirstWord).get(0).props.children).toEqual('');
			expect(wrapper.find(Nav).find(StyledNavItem).at(0).find(TitleRest).get(0).props.children).toEqual('Open Orders (0)');
			expect(wrapper.find(Nav).find(StyledNavItem).at(1).find(TitleFirstWord).get(0).props.children).toEqual('Your ');
			expect(wrapper.find(Nav).find(StyledNavItem).at(1).find(TitleRest).get(0).props.children).toEqual('Debts (0)');
			expect(wrapper.find(Nav).find(StyledNavItem).at(2).find(TitleFirstWord).get(0).props.children).toEqual('Your ');
			expect(wrapper.find(Nav).find(StyledNavItem).at(2).find(TitleRest).get(0).props.children).toEqual('Investments (0)');
		});
		...
		it('should render a <OpenOrdersContainer />', () => {
			expect(wrapper.find(TabContent).find(TabPane).at(0).find(OpenOrders).length).toEqual(1);
		})

		it('should render a <DebtsContainer />', () => {
			expect(wrapper.find(TabContent).find(TabPane).at(1).find(DebtsContainer).length).toEqual(1);
		})

		it('should render an <InvestmentsContainer />', () => {
			expect(wrapper.find(TabContent).find(TabPane).at(2).find(InvestmentsContainer).length).toEqual(1);
		})
	});
