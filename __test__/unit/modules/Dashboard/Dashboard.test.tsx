...
describe('<Dashboard />', () => {
	describe('#render', () => {
		...
		it('should render the correct title', () => {
			expect(wrapper.find(Nav).find(StyledNavItem).first().find(TitleFirstWord).get(0).props.children).toEqual('Your ');
			expect(wrapper.find(Nav).find(StyledNavItem).first().find(TitleRest).get(0).props.children).toEqual('Debts (0)');
			expect(wrapper.find(Nav).find(StyledNavItem).last().find(TitleFirstWord).get(0).props.children).toEqual('Your ');
			expect(wrapper.find(Nav).find(StyledNavItem).last().find(TitleRest).get(0).props.children).toEqual('Investments (0)');
		});
		...
		it('should render a <DebtsContainer />', () => {
			expect(wrapper.find(TabContent).find(TabPane).first().find(DebtsContainer).length).toEqual(1);
		})

		it('should render an <InvestmentsContainer />', () => {
			expect(wrapper.find(TabContent).find(TabPane).last().find(InvestmentsContainer).length).toEqual(1);
		})
	});
